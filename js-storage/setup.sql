-- The fixture storage-js's own integration tests are written against.
--
-- Upstream keeps it at packages/core/storage-js/test/supabase/seed.sql
-- in github.com/supabase/supabase-js, where the supabase CLI applies it
-- during `supabase db reset`. It is applied over a connection here,
-- because there is no CLI in the loop.
--
-- One difference from upstream: the drops at the top, so the file can
-- be applied twice to the same database. Everything under them is
-- upstream's, rows and policies alike, because the policies are half of
-- what the object tests are asking about.
--
...
-- hashes want pgcrypto in the extensions schema, which zou's bootstrap
-- creates, tamnd/zou#214.
...
-- than dropping the trigger and putting it back.
set storage.allow_delete_query = 'true';

drop policy if exists crud_buckets on storage.buckets;
drop policy if exists crud_objects on storage.objects;
drop policy if exists crud_public_folder on storage.objects;
drop policy if exists crud_public_file on storage.objects;
drop policy if exists crud_uid_folder on storage.objects;
drop policy if exists crud_uid_file on storage.objects;
drop policy if exists authenticated_folder on storage.objects;
drop policy if exists crud_owner_only on storage.objects;
drop policy if exists open_all_update on storage.objects;
drop policy if exists crud_my_bucket on storage.objects;
