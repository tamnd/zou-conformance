// storage-js's own integration tests, pointed somewhere else.
//
// Upstream keeps this at packages/core/storage-js/test/ in
// github.com/supabase/supabase-js and runs it against a local stack
// brought up with `supabase start`. It is a statement of what the
// client needs from the storage server, written by the people who
// wrote the client, so it is worth running unchanged rather than
// paraphrasing into questions of our own.
//
// The differences from upstream in this file, and nothing else: the
// three tests that ask the server to actually transform an image are
// gated behind an environment flag, because zou does not serve
// /render/image yet, and a suite that fails on a feature nobody has
// written is noise rather than a measurement. Upstream skips two more
// of them itself. Set ZOU_IMAGE_TRANSFORMS=1 when there is something
// to ask, and they come back exactly as upstream wrote them.
//
// Nothing else moves. No assertion is edited. A failure here is zou
// answering differently from the server the test was written against.
...
// tamnd/zou#3: image transforms are the part of /storage/v1 that is
// not built. The three tests below are the ones that need a real one.
const transforms = process.env.ZOU_IMAGE_TRANSFORMS === '1' ? it : it.skip
...
describe('Object API', () => {
  ...
  describe('Transformations', () => {
    ...
    transforms('will download an authenticated transformed file', async () => {
  ...
  transforms('will get a signed transformed image', async () => {
    await storage.from(bucketName).upload(uploadPath, file)
...
describe('download with fetch parameters', () => {
  ...
  transforms('download with transform and fetch parameters', async () => {
    const uploadRes = await storage.from(bucketName).upload(uploadPath, file)
    expect(uploadRes.error).toBeNull()
